...
/**
 * 방 참여하기 — 전달받은 <b>초대 코드만</b>으로 방장 서버에 붙습니다.
 *
 * <p>접속 주소를 찾는 방법이 두 단계입니다.
 * <ol>
 *   <li><b>코드에서 꺼내 바로 접속</b>({@link InviteCode}) — 주소가 코드 안에 들어 있어 즉시 붙습니다.
 *       평소에는 여기서 끝나고, 네트워크에 아무것도 뿌리지 않습니다.</li>
 *   <li><b>자동 탐색</b>({@link RoomFinder}) — 1차가 실패할 때만 씁니다. 방장이 와이파이를 다시 잡아
 *       IP가 바뀌면 코드에 굳어 있는 주소가 낡은 값이 되는데, 같은 네트워크에 질의를 뿌려
 *       <b>현재 주소</b>를 받아 옵니다. 참여자가 코드를 다시 받을 필요가 없습니다.</li>
 * </ol>
 *
 * <p>어느 쪽으로 붙었든 입장 성공 여부는 서버의 JOINED / ERROR 응답으로 판정합니다 —
 * 주소를 찾은 것과 입장 자격이 있는 것은 별개라, 코드의 시크릿 검사는 서버가 따로 합니다.
 */
public class JoinRoomController {
    ...
    private void initialize() {
        ...
        codeField.textProperty().addListener((observable, old, text) -> updatePreview(text));
        updatePreview("");
    }
    ...
    private void updatePreview(String text) {
        ...
        if (code.length() < InviteCode.LENGTH) {
            previewLabel.setText(code.length() + " / " + InviteCode.LENGTH + "자");
            previewLabel.getStyleClass().remove("error-text");
            return;
        }
        try {
            previewLabel.setText("방장 주소 " + InviteCode.parse(code).address());
            previewLabel.getStyleClass().remove("error-text");
    ...
    private void handleJoin() {
        InviteCode invite;
        try {
            invite = InviteCode.parse(codeField.getText());   // 여기서 주소가 나옵니다
        } catch (IllegalArgumentException e) {
    ...
    private void fail(String reason) {
        responseTimeout.stop();
        setBusy(false);
        updatePreview(codeField.getText());   // 진행 상황 자리를 원래 안내로 되돌립니다
